...
public partial class FluidSimulation : Node2D
{
	...
	public float nearpressureMultiplier = 1000.0f;
	...
	public float grid_cell_size = 20.0f;

	private List<Particle> particles = new List<Particle>();

private Dictionary<Vector2, List<Particle>> grid = new Dictionary<Vector2, List<Particle>>();
	private Vector2 GetCellIndex(Vector2 position)
	{
		return new Vector2(Mathf.Floor(position.X / grid_cell_size), Mathf.Floor(position.Y / grid_cell_size));
	}
	private void AssignParticlesToGrid()
	{
		...
		{
			Vector2 cell = GetCellIndex(particle.Position);
	...
	private List<Particle> GetNeighbors(Particle particle)
	{
		List<Particle> neighbors = new List<Particle>();
		float radius = smoothingRadius;
		Vector2 cellIndex = GetCellIndex(particle.Position);
		int minCellX = Mathf.FloorToInt((particle.Position.X - radius) / grid_cell_size);
		int maxCellX = Mathf.FloorToInt((particle.Position.X + radius) / grid_cell_size);
		int minCellY = Mathf.FloorToInt((particle.Position.Y - radius) / grid_cell_size);
		int maxCellY = Mathf.FloorToInt((particle.Position.Y + radius) / grid_cell_size);
		...
		for (int x = minCellX; x <= maxCellX; x++)
		{
			for (int y = minCellY; y <= maxCellY; y++)
			{
				...
				if (grid.ContainsKey(cell))
				{
					foreach (var neighbor in grid[cell])
					{
						if (neighbor.Position.DistanceTo(particle.Position) <= radius)
						{
...
;
	...
	private void CalculateDensity(){
		...
		{
			particle.density = 0f;
			var neighbors = GetNeighbors(particle);
			foreach (var neighbor in neighbors)
			{
				Vector2 Vec = neighbor.GetPredictedPosition(deltaTime) - particle.GetPredictedPosition(deltaTime);
				float VecLen = Vec.Length();
				if (VecLen <= smoothingRadius)
				{
	...
	private void ApplyPressureForce(){
		foreach(var particle in particles){
			...
			foreach(var neighbor in neighbors){
				...
				viscosityForce += (neighbor.velocity - particle.velocity)* Poly6Kernel(VecLen) * viscosity;
			}
			particle.ApplyForce(pressureForce/particle.density, deltaTime);
			particle.ApplyForce(viscosityForce/particle.density, deltaTime);
